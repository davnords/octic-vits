...
def trigger_job_requeue_handler(signum, frame):
    if torch.distributed.get_rank() == 0:
        print(f"got signal {signum}, requeuing")
        subprocess.check_call(["scontrol", "requeue", os.environ["SLURM_JOB_ID"]])
        print("New job submitted to the queue")
...
def get_params_groups(
    ...
    for name, param in model.named_parameters():
        ...
        print(f"{d['name']}:   " + ", ".join([f"{k}: {v}" for k, v in d.items() if k not in ["param", "name"]]))
    # FU
...
class VerboseShardedGradScaler(ShardedGradScaler):
    def _maybe_opt_step(self, optimizer, optimizer_state, *args, **kwargs):
        ...
        if self._verbose_iter % 1000 == 0:
            print(f"grad scaler scale: {self.get_scale()}")
        if not sum(v.item() for v in optimizer_state["found_inf_per_device"].values()):
            return optimizer.step(*args, **kwargs)
        scale = self.get_scale()
        print(f"Grad scaler scale: {scale}")
        if scale == 0.0:
            raise RuntimeError("grad scaler scale is 0.0, the run is dead")
        print("Found inf, skipping update")
        return float("inf")
...
class MetricLogger:
    ...
    def log_every(self, iterable, print_freq, header=None, n_iterations=None, start_iteration=0):
        ...
        print(f"{header} Total time: {total_time_str} ({total_time / n_iterations:.6f} s / it)")
...
def enable_distributed(
    *,
    ...
    if "SLURM_JOB_ID" in os.environ and not os.environ.get("TORCH_DIST_FORCE_USE_ENV", False):
        print("Dist init from Slurm environment")
        ...
        print("Dist init for single-gpu training")
        ...
        print("Dist init from preset env")
...
def get_shared_libraries():
    try:
        ...
        print(f"libcudart symlink failed: {e}")
