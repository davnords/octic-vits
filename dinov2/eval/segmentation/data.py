...
def download_dataset(
    ...
    with FileLock(root.with_suffix(".lock"), mode=0o666):
        ...
        if not root.exists():
            root.mkdir()
            print(f"Dataset not found, downloading {url}")
...
class ADE20K(DownloadedSegDataset):
    def __init__(
        ...
        print(f"Created dataset {self!r}")
...
def make_dataset(
    ...
    print(f"Creating dataset {dataset_str_or_path}")
    ...
    if parsed.scheme == "hf":
        ...
        print(f"Using HuggingFace dataset: {ds_name} with kwargs {kwargs}")
    ...
    if parsed.scheme == "torchvision":
        print(f"Using torchvision.datasets.{parsed.netloc}")
        if "root" not in kwargs:
...
def make_data_loader(
    *,
    ...
    if isinstance(dataset, torch.utils.data.IterableDataset):
        print("Dataset is iterable, not using a sampler")
        sampler = None
    elif infinite:
        assert shuffle, "Infinite sampler requires shuffle"
        print("Using InfiniteSampler")
