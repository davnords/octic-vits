...
from dinov2.eval.segmentation.data import DatasetWithEnumeratedTargets, make_data_loader, make_dataset
from dinov2.eval.segmentation.utils import IMAGENET_NORM, MetricLogger, base_setup, dump_metrics, get_num_classes, import_path
...
def make_padded_dataloader(batch_size, num_workers, dataset, tag: str):
    ...
    print(f"Num batches {tag}: {len(dataloader)} this rank")
    return dataloader
...
def eval_model(
    ...
    print(f"Num samples in train_dataset: {len(train_dataset):_}")
    print(f"Num samples in val_dataset: {len(val_dataset):_}")
    img, target = train_dataset[0]
    print(f"Sample img: {img.shape=}")
    ...
    if auto_resume and last_ckpt_link.exists():
        print(f"Resuming from {last_ckpt_link=}")
    ...
    print(f"Eval val_dataset at iteration {n_iters}")
    ...
    for test_dataset_name in test_dataset_names:
        print(f"Eval on {test_dataset_name} at iteration {n_iters}")
        ...
        print(f"Num samples in test_dataset: {len(test_dataset):_}")
...
def main(cfg) -> dict[str, float]:
    ...
    try:
        ...
        print(f"Total time: {datetime.timedelta(seconds=round(end - start))}")
    return results_dict
