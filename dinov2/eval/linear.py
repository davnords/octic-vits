...
def get_args_parser(
    ...
    parser.set_defaults(
        train_dataset_str=f"ImageNet:split=TRAIN:root={constants.IMAGENET_PATH}:extra={constants.IMAGENET_PATH}/extra",
        val_dataset_str=f"ImageNet:split=VAL:root={constants.IMAGENET_PATH}:extra={constants.IMAGENET_PATH}/extra",
...
def evaluate_linear_classifiers(
    ...
    for i, (classifier_string, metric) in enumerate(results_dict_temp.items()):
        print(f"{prefixstring} -- Classifier: {classifier_string} * {metric}")
        if (
    ...
    for data, labels in metric_logger.log_every(
        ...
        if iteration - start_iter > 5:
            if iteration % running_checkpoint_period == 0:
                torch.cuda.synchronize()
                if distributed.is_main_process():
                    print("Checkpointing running_checkpoint")
...
def test_on_datasets(
    ...
    for test_dataset_str, class_mapping, metric_type in zip(test_dataset_strs, test_class_mappings, test_metric_types):
        print(f"Testing on {test_dataset_str}")
...
def run_eval_linear(
    ...
    if val_class_mapping_fpath is not None:
        print(f"Using class mapping from {val_class_mapping_fpath}")
    ...
    for class_mapping_fpath in test_class_mapping_fpaths:
        if class_mapping_fpath is not None and class_mapping_fpath != "None":
            print(f"Using class mapping from {class_mapping_fpath}")
    ...
    print("Test Results Dict " + str(results_dict))
...
def main(args):
    ...
    args.output_dir = args.dir + f"/eval/training_{ckpt_iter}/linear"
