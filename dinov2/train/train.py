...
For python-based LazyConfig, use "path.key=value".
    ...
    parser.add_argument(
        "--dont-log-wandb",
        action="store_true",
        help="Whether to log to wandb or not. ",
    )
...
def do_train(cfg, model, resume=False, dont_log_wandb=True):
    model.train()
    ...
    if OFFICIAL_EPOCH_LENGTH <= 0:
        OFFICIAL_EPOCH_LENGTH = len(dataset) // (cfg.train.batch_size_per_gpu * distributed.get_global_size())
    ...
    if not dont_log_wandb and distributed.is_main_process():
        print("Running experiments with wandb on the first GPU")
        wandb.init(
            name=cfg.student.arch,
            project="DINOv2",
            entity="georgs-team",
            config={
                'batch_size': cfg.train.batch_size_per_gpu,
...
def main(args):
    cfg = setup(args)
    print('Training model: {}'.format(cfg.student.arch))
    ...
    do_train(cfg, model, resume=not args.no_resume, dont_log_wandb=args.dont_log_wandb)
