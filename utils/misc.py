...
def invariarize(xs: Tuple[torch.Tensor], method: Literal["a1", "spectrum"]='spectrum') -> torch.Tensor:
    """ Invariarize the input tensor
    Args:
        xs: tuple of tensors
        method: invariarization method
    Returns:
        Tensor of invariant features
    """
    if method == "a1":
        return xs[0]
        ...
        return torch.cat(
            (
                ...
                torch.abs(xs[2]),                       # B1
                torch.abs(xs[3]),                       # B2
                xs[4]**2 + xs[5]**2,                    # E norm 1
                xs[6]**2 + xs[7]**2,                    # E norm 2
                xs[4] * xs[6] + xs[5] * xs[7],          # E dot product
                torch.abs(xs[4]*xs[7] - xs[5]*xs[6]),   # abs of E determinant,
            ),
            dim=-1,
        )
    else:
        raise ValueError(f"Unknown invariarization method: {method}")